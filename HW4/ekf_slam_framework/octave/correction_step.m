...
for i = 1:m
  ...
  Fxj = zeros(5,2*N+3);
  Fxj(1:3,1:3) = eye(3,3);
  Fxj(4:5,2*landmarkId+2:2*landmarkId+3) = eye(2,2);
...
### Sequential update
  ...
  K = sigma*Hi'*inv(Hi*sigma*Hi' + Qt);
  e = Z(2*i-1:2*i)-expectedZ(2*i-1:2*i) ;
  e = normalize_all_bearings(e);
  
  #K1 = K1 + K*e;
  #K2 = K2+K*Hi;
  
  mu = mu+K*e;
  mu(3) = normalize_angle(mu(3)); #Not repeatedly linearizing can lead to divergence when updating sequentially
  
  sigma = (eye(size(sigma))-K*Hi)*sigma;
...
% TODO: Compute the Kalman gain
m
K = sigma*H'/(H*sigma*H' + Q); #Batch update http://robots.engin.umich.edu/mobilerobotics/media/Prof_Eustice_slides/L11.EKF-SLAM-I.pdf
% TODO: Compute the difference between the expected and recorded measurements.
% Remember to normalize the bearings after subtracting!
% (hint: use the normalize_all_bearings function available in tools)
